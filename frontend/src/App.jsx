...
function App() {
  ...
  const handleAddSiteSubmit = async (e) => {
    ...
    try {
      ...
      // Resetta il form
      setNewSiteData({
        site_code: '', Nome: '', Indirizzo: '', Regione: '', Provincia: '', Comune: '', 'Tipo struttura': '', Ownership: '', 'Classe 2025': '', 'Classe 2019': '', 'Delta Classe': '', Latitudine: '', Longitudine: ''
      });
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '5px', width: '500px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 5px 15px rgba(0,0,0,0.3)' }}>
          ...
          <form onSubmit={handleAddSiteSubmit} style={{ padding: '20px', overflowY: 'auto', flex: 1 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 15px' }}>
              ...
              <div>
                <label style={labelStyle}>Classe 2019</label>
                <input type="text" name="Classe 2019" value={newSiteData['Classe 2019']} onChange={handleInputChange} style={inputStyle} placeholder="Es. 1, 2, 3..." />
              </div>
              ...
              <div>
                <label style={labelStyle}>Delta Classe</label>
                <input type="text" name="Delta Classe" value={newSiteData['Delta Classe']} onChange={handleInputChange} style={inputStyle} placeholder="Es. +1, -2..." />
              </div>
              ...
              {sites.map((site, index) => {
                ...
                return (
                  <tr key={site.site_code} style={{ backgroundColor: rowBg, borderBottom: '1px solid #ddd' }}>
                    ...
                    <td style={{ padding: '12px 15px', color: '#555' }}>{data.Denominazione || data.Nome || data.Descrizione || '---'}</td>
    ...
    return (
      <div key={site.site_code} style={{ backgroundColor: '#ffffff', borderBottom: '1px solid #ddd', padding: '15px', cursor: 'pointer' }} onClick={() => setSelectedSite(site)}>
        <h3 style={{ margin: '0 0 5px 0', fontSize: '16px', fontWeight: 'bold', color: '#000' }}>
          [{site.site_code}]{data.Denominazione || data.Nome || data.Descrizione || 'Sito senza nome'}
        </h3>
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <style>
          ...
          .header-container {
            background: linear-gradient(to bottom, #68c8e3, #5bc0de);
      ...
      <body>
        <div class="header-container">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
            ...
            <div style="display: flex; align-items: center; gap: 16px;">
              ...
              <button onclick="downloadCSV()" style="background: white; color: #5bc0de; border: 1px solid #cccccc; padding: 4px 12px; font-size: 12px; font-weight: bold; border-radius: 4px; cursor: pointer; box-shadow: 0 1px 2px rgba(0,0,0,0.2);">Scarica CSV</button>
            </div>
          </div>
          <div class="tabs" style="display: flex; flex-wrap: wrap;">
              ...
              <button class="tab-btn" onclick="openTab('classificazione', this)">Switchoff</button>
              <button class="tab-btn" onclick="openTab('sicurezza', this)">Decommissioning</button>
        ...
        <div class="content-area">
          <div class="sub-header">
            <h2 class="sub-title">${data.Denominazione || data.Nome || data.Descrizione || 'Dettaglio Sito'}</h2>
          ...
          <div id="anagrafica" class="tab-content active">
            <div class="grid-layout">
              ...
              ${buildRow('Indirizzo:', data.Address || data.Indirizzo, false, '', 'Address')}
              ${buildRow('Regione:', data.Region || data.Regione, false, '', 'Region')}
              ${buildRow('Provincia:', data.Province || data.Provincia, false, '', 'Province')}
              ${buildRow('Comune:', data.City || data.Comune, false, '', 'City')}
              ${buildRow('Territorial Area:', data['Territorial Area'], false, '', 'Territorial Area')}
              ${buildRow('Building / Terrain:', data['Building or Terrain'] || data['Tipo struttura'], false, '', 'Building or Terrain')}
              ${buildRow('Ownership:', data.Ownership || data.SocietaImmobile, false, '', 'Ownership')}
              ${buildRow('CLASSE 2019:', data['Classe 2019'] || data['CLASSE 2019'], false, '', 'CLASSE 2019')}
              ${buildRow('CLASSE 2025:', data['Classe 2025'] || data['CLASSE 2025'], false, '', 'CLASSE 2025')}
              ${buildRow('DELTA CLASSE:', data['Delta Classe'] || data['DELTA CLASSE'], false, '', 'DELTA CLASSE')}
          ...
          <!-- TAB SICUREZZA ORA DECOMMISSIONING -->
          <div id="sicurezza" class="tab-content">
            <div class="grid-layout">
              ${buildRow('Decomm 2026 (Mese fine lav.):', data['Decomm 2026 (Mese di fine lavorazione)'] || data['Mese di fine lavorazione'], false, '', 'Decomm 2026 (Mese di fine lavorazione)')}
              ${buildRow('Decomm 2026 (Cluster. rel3):', data['Decomm 2026 (Clusterizzazione rel3)'] || data['Clusterizzazione rel3'], false, '', 'Decomm 2026 (Clusterizzazione rel3)')}
              ${buildRow('Decomm 2026 (PoP/Bypass/LT):', data['Decomm 2026 (PoP/Bypass/LT)'] || data['PoP/Bypass/LT'], false, '', 'Decomm 2026 (PoP/Bypass/LT)')}
              ${buildRow('Stima KM estraibile:', data['Stima KM estraibile'], false, '', 'Stima KM estraibile')}
              ${buildRow('Stima tonn estraibile:', data['Stima tonn estraibile'], false, '', 'Stima tonn estraibile')}
            </div>
          </div>
        ...
        <script>
          ...
          function downloadCSV() {
            ...
            const values = keys.map(k => {
              ...
              // Rimuove eventuali ritorni a capo dai valori e formatta per CSV
              val = String(val).replace(/[\\n\\r]+/g, ' ').replace(/"/g, '""');
              return '"' + val + '"';
            });
            
            // Pulisce le chiavi dai ritorni a capo nascosti
            const cleanKeys = keys.map(k => '"' + String(k).replace(/[\\n\\r]+/g, ' ') + '"');
            
            const csvContent = cleanKeys.join(',') + '\\n' + values.join(',');
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <aside style={{ width: '220px', backgroundColor: '#f9f9f9', borderRight: '1px solid #e7e7e7', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', height: '40px' }}>
            <button style={{ flex: 1, backgroundColor: '#5cb85c', border: 'none', borderRight: '1px solid #4cae4c', color: 'white', cursor: 'pointer' }} onClick={() => { setCurrentPage('home'); setViewMode('table'); }}>📋</button>
            <button style={{ flex: 1, backgroundColor: '#5bc0de', border: 'none', borderRight: '1px solid #46b8da', color: 'white', cursor: 'pointer' }} onClick={() => { setCurrentPage('home'); setViewMode('card'); }}>🌍</button>
            <button style={{ flex: 1, backgroundColor: '#d9534f', border: 'none', color: 'white', cursor: 'pointer' }} onClick={() => setCurrentPage('stats')}>📊</button>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            ...
            <div style={{ padding: '15px', backgroundColor: '#fff' }}>
              ...
              <form onSubmit={handleFileUpload} style={{ display: 'flex', flexDirection: 'column', gap: '10px', borderTop: '1px solid #eee', paddingTop: '10px' }}>
                ...
                <div>
                  <div style={{ fontSize: '10px', color: '#777', marginBottom: '2px' }}>ClassPointSUD_con_coordinate (Excel/CSV)</div>
                  <input type="file" id="file-data" style={{ fontSize: '10px', width: '100%' }} onChange={(e) => setFileData(e.target.files[0])} />
                </div>
