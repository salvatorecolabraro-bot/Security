...
const Statistics = ({ sites, onBack }) => {
  const stats = useMemo(() => {
    ...
    sites.forEach(site => {
      ...
      // Edifici / Terreni
      const tipo = (data['Building or Terrain'] || data['Tipo struttura'] || '').toLowerCase().trim();
      if (tipo.includes('edifici') || tipo.includes('building') || tipo === 'edificio') {
        edifici++;
      } else if (tipo.includes('terren') || tipo.includes('terrain') || tipo === 'terreno') {
        terreni++;
      }
      ...
      // Remote Survey (Conteggio per le card: conta se non vuoto/non null)
      const valRemoteSurvey = data['REMOTE SURVEY'] || data['REMOTE SURVEY '] || data['Remote Survey'];
      if (valRemoteSurvey !== null && valRemoteSurvey !== undefined && String(valRemoteSurvey).trim() !== '') {
        totRemoteSurvey++;
      }
    ...
    sites.forEach(site => {
      ...
      if (tipo.includes('edifici') || tipo.includes('building') || tipo === 'edificio') {
        edificiSites.push(site);
      } else if (tipo.includes('terren') || tipo.includes('terrain') || tipo === 'terreno') {
        terreniSites.push(site);
      }
    });
      ...
      <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', marginBottom: '40px' }}>
        ...
        <div style={{ flex: '1 1 400px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            ...
            <StatCard icon="📡" color="#8e959b" count={stats.totRemoteSurvey} label="Remote Survey" badgeCount={0} />
          </div>
